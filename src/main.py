...
6) (Optionnel) Visualisation Matplotlib et export PyMOL (.pml).
"""
...
def traiter_un_pdb(
    ...
    rsa_thresh: float,           # << ajouté
    min_half: float | None,      # << ajouté (borne demi-épaisseur)
    max_half: float | None,      # << ajouté (borne demi-épaisseur)
) -> None:
    """Exécute le pipeline complet sur un PDB et affiche un récapitulatif détaillé."""
    print(f"\n=== {pdb_path.name} ===")
    ...
    for normal in dirs:
        ...
        # --- Bornes réalistes de demi-épaisseur (désactivables en mettant None) ---
    ...
    for k, (Q, n_u, c, w, d_in, d_out) in enumerate(dir_scores, start=1):
        ...
        print(
            f"{k:2d}. Q={Q:.3f} | n={n_str} | centre={c:.2f} Å | "
            f"demi-ép={w:.2f} Å | d_in={d_in:.2f} Å | d_out={d_out:.2f} Å"
        )
    ...
    print(
        f"- Centres des dalles (3D)      : "
        f"IN  = [{center_in[0]:.3f}, {center_in[1]:.3f}, {center_in[2]:.3f}] | "
        f"OUT = [{center_out[0]:.3f}, {center_out[1]:.3f}, {center_out[2]:.3f}]"
    )
    ...
    if pml_out:
        ...
        print(f"- Script PyMOL écrit : {pml_out}")
...
def main() -> None:
    ...
    ap.add_argument(
        "--membrane-dir",
        type=Path,
        default=Path("data/membrane"),
        help="Dossier de PDB membranaires (batch si --pdb absent).",
    )
    ap.add_argument(
        "--globular-dir",
        type=Path,
        default=Path("data/globular"),
    ...
    ap.add_argument(
        "--dirs-first",
        action="store_true",
        help="Si --plot, affiche COM+directions avant la phase score.",
    )
    ap.add_argument("--pml", type=str, default=None, help="Écrit un script PyMOL (.pml)")
    ap.add_argument(
        "--head",
        type=int,
        default=0,
    ...
    # --- Nouveaux garde-fous / options ---
    ap.add_argument(
        "--rsa-thresh",
        type=float,
        default=0.20,
        help="Seuil minimal sur la pondération (RSA ou 1−RSA) pour contribuer au score.",
    )
    ap.add_argument(
        "--no-widen",
        action="store_true",
        help="Désactive tout élargissement (équivaut à --max-half-extra 0).",
    )
    ap.add_argument(
        "--min-half",
        type=float,
        default=10.0,
        help="Borne basse (Å) de la demi-épaisseur (None pour désactiver : mettre -1).",
    )
    ap.add_argument(
        "--max-half",
        type=float,
        default=20.0,
        help="Borne haute (Å) de la demi-épaisseur (None pour désactiver : mettre -1).",
    )
    # -------------------------------------
    ...
    # Permettre de désactiver les bornes via -1
